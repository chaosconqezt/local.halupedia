...
Иногда, по своему усмотрению, если позволяет временной промежуток, пиши статью в дореволюционном стиле (с ятями 'ѣ', ижицами 'ѵ', фитами 'ѳ', ерами 'ъ' на конце слов) или в архаичном летописном древнерусском стиле, чтобы разнообразить контент. Это должно выглядеть грандиозно и естественно в контексте бредовых исторических статей.
Также ВСЕГДА активно используй устаревшие слова и анахронизмы из этого списка (а также подобные им): барин, изба, губерния, городовой, околоточный, челобитная, сударь, подьячий, кафтан, стряпчий, урядник. 
Также обязательно используй старорусские меры длины, веса и денег (аршин, пуд, верста, алтын, грош, сажень) вместо современных. Вплетай их в текст естественно и к месту, ни в коем случае НЕ перечисляя их подряд. Это придаст тексту старомодный бюрократический колорит.
...
- The context value is a single dense sentence in Russian (10–25 words) stating: (a) what the entity is — person, place, object, concept, ritual, organization, etc.; (b) its century / era / period; (c) its specific role or relation to the current article. Be concrete: invent dates, professions, geographic placements, instruments. NEVER use double quotes inside context (use commas or single quotes if needed). NEVER use raw < or > inside context. Examples (do not copy verbatim):
  context='губернский стряпчий 19 века, составитель челобитной о запрете облаков'
  context='латунный аршин, использовавшийся при анатолийской переписи пудов овса, вышел из употребления с 1922 года'
  context='околоточный надзиратель, служивший в 1881–1934 годах, назначенный следить за порядком на верстовых столбах'
...
export function buildUserMessage(opts: GenerateOptions): string {
  const lines = [
    ...
    `The canonical URL slug for this article is /${opts.slug}, but when writing the article's title and body you must use normal spaces: "${opts.title}".`,
    `Do NOT use underscores in the article text or <h1> title, ONLY in the <a href="..."> attributes.`,
  ];
